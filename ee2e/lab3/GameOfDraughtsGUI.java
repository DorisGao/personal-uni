...
class GameOfDraughtsFrame extends JFrame implements ActionListener
{
	...
	public GameOfDraughtsFrame()
	{
		...
		* Add the menu bar
        ...
        BlackMoveItem=new JMenuItem("Move Black");
        BlackMoveItem.addActionListener(this);
		GameMenu.add(BlackMoveItem);

		WhiteMoveItem=new JMenuItem("Move White");
		WhiteMoveItem.addActionListener(this);
		GameMenu.add(WhiteMoveItem);
        GameMenu.addSeparator();
	...
	public void actionPerformed(ActionEvent evt)
	{
        ...
        else if ((source==NextMoveItem)&&(!endOfGame))
        {
          ...
          ***/
	      ...
	      {
		    whitePlayer.movePiece(board, DraughtBoard.WHITE);
		    nextMoveColor=DraughtBoard.BLACK;
		  }
		  else
		  {
		    blackPlayer.movePiece(board, DraughtBoard.BLACK);
		    nextMoveColor=DraughtBoard.WHITE;
		  }

		  int ws=board.getWhiteScore();
		  int bs=board.getBlackScore();
		...
		{
		  System.out.println("BasicWhiteItem Not implemented fully");
		  /* Plug in basic white player here */
		  whitePlayer = new Player();
		}
		else if (source==AdvancedWhiteItem)
		{
		  System.out.println("AdvancedWhiteItem Not implemented fully");
		  /* Plug in advanced white player here */

		}
		else if (source==BasicBlackItem)
		{
		  System.out.println("BasicBlackItem Not implemented fully");
		  /* Plug in basic black player here */
		  blackPlayer = new Player();
		}
		else if (source==AdvancedBlackItem)
		{
		  System.out.println("AdvancedBlackItem Not implemented fully");
		  /* Plug in advanced black player here */
		}

		else if (source==BlackMoveItem)
		{
			// System.out.println("BlackMoveItem Not implemented");
			board.moveBlack(0, 4, 'l');
			board.moveBlack(1, 5, 'l');
			board.moveBlack(0, 4, 'r');
			int ws=board.getWhiteScore();
			int bs=board.getBlackScore();
		  	boardPanel.draw(ws,bs);	// re-draw the board
			/* test move black */
		}
		else if (source==WhiteMoveItem)
		{
			board.moveWhite(2, 2, 'l');
			int ws=board.getWhiteScore();
			int bs=board.getBlackScore();
		  	boardPanel.draw(ws,bs);	// re-draw the board
			/* test move white */
	...
	private JMenuItem BlackMoveItem, WhiteMoveItem;
